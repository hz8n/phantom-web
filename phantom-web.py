#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# Aegis Web Sentinel — phantom-web v1.0.0
...
BANNER = f"""{C}{BOLD}  ◈◈◈  Aegis Web Sentinel  ◈◈◈{RST}
{C}  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━{RST}
  {W}phantom-web • Web Audit • v1.0.0 • Design by al3rab • Full English{RST}
{M}  Headers + title + tech stack + security header score{RST}
"""

def get_info():
    try: ip=socket.gethostbyname(socket.gethostname())
    except: ip="127.0.0.1"
    return {"host":socket.gethostname(),"ip":ip,"os":platform.system(),"time":datetime.datetime.now().isoformat(),"tool":"phantom-web","title":"Aegis Web Sentinel","category":"Web Audit","designer":"al3rab","lang":"English"}

def main():
    p=argparse.ArgumentParser(description="Aegis Web Sentinel - Headers + title + tech stack + security header score | Design by al3rab")
    ...
    args=p.parse_args()
    print(BANNER)
    print(f"{G}[✓] Legendary advanced engine armed{RST}  {GR}phantom-web v1.0.0 • Design by al3rab{RST}")
    ...
    if not is_private and not args.allow_public:
        ...
        sys.exit(1)
    data=get_info()
    data.update({"target":args.target,"findings":["Headers + title + tech stack + security header score - advanced finding 1","Legendary terminal check passed"],"risk":"LOW","legendary":True,"advanced":True})
